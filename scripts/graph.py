...
def main():
    communities = nx.community.louvain_communities(G, weight='weight', resolution=3, seed=69)
    ...
    # colors = []
    # for node in G.nodes:
    #     print(f"{node}: {nx.degree(G, nbunch=node)}")
    #     colors.append
    pos = nx.spring_layout(G, weight='weight', k=0.2, seed=7)
